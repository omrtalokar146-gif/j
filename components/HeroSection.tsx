...
export default function HeroSection({ onPlayGame, trendingGames, featuredGames, setActiveTab }: HeroSectionProps) {
  ...
  const [shuffledTrending, setShuffledTrending] = useState<Game[]>(() => shuffleGames(trendingGames));

  useEffect(() => {
    setShuffledTrending(shuffleGames(trendingGames));
  }, [trendingGames]);
  ...
  const platformStats = [
    ...
    { value: '1200+', label: 'Games Available', icon: <Target size={14} className="text-pink-400" /> },
  ];
    ...
    <div className="w-full max-w-7xl mx-auto px-6 md:px-12 py-10 md:py-20 relative z-10 flex flex-col gap-16 md:gap-24">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        ...
        <div className="lg:col-span-7 flex flex-col gap-6 text-left">
          ...
          <div className="inline-flex items-center gap-2 rounded-full bg-white/5 border border-cyan-400/20 text-cyan-200 px-4 py-2 text-sm font-semibold mt-4 max-w-max">
            <Target size={14} className="text-cyan-300" /> 1200+ Games available now on the Nexus portal
          </div>
