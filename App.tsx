...
const isAdminUser = (username?: string): boolean => {
  ...
  const adminUsernames = ['omy134567'];
...
export default function App() {
  ...
  const [liveSocketConnections, setLiveSocketConnections] = useState<number>(0);
  const [muteSound, setMuteSound] = useState(audioSystem.isMuted());
  const [authError, setAuthError] = useState('');

  const activeUsersCount = accounts.filter((account: StoredAccount) => account.isLoggedIn).length;
  const activeGamePlayersCount = accounts.filter((account: StoredAccount) => account.currentGameId).length;
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && BACKEND_URL) {
      (async () => {
        try {
          const res = await fetch(`${BACKEND_URL.replace(/\/$/, '')}/api/metrics/pageview`, {
            method: 'POST',
          });
          if (res.ok) {
            const data = await res.json();
            if (typeof data.count === 'number') {
              setPageViewsCount(data.count);
            }
          }
        } catch (error) {
  ...
  useEffect(() => {
    if (!BACKEND_URL || activeTab !== 'admin') return;

    (async () => {
      try {
        const res = await fetch(`${BACKEND_URL.replace(/\/$/, '')}/api/metrics/pageviews`);
        if (res.ok) {
          const data = await res.json();
          if (typeof data.count === 'number') {
            setPageViewsCount(data.count);
        ...
        console.warn('Failed to fetch page view total', error);
      }
    })();
  }, [activeTab]);
  ...
  useEffect(() => {
    ...
    const loadSocketIoClient = async () => {
      if (!window.io) {
      ...
      socket.on('liveConnections', (count: number) => {
        setLiveSocketConnections(count);
      });
  ...
  const handleSignIn = (username: string, password: string, isSignUp: boolean) => {
    ...
    const isAdminOverride = trimmedUsername.toLowerCase() === 'omy134567' && password === 'om134567';
    if (isAdminOverride) {
      const profile = createProfile(trimmedUsername);
      profile.email = `${trimmedUsername}@nexus.local`;
      const nextAccount = {
      ...
      const nextAccounts = existingIndex !== -1
        ? accountList.map((account, idx) => idx === existingIndex ? nextAccount : account)
        : [...accountList, nextAccount];
      saveRegisteredAccounts(nextAccounts);
      setAccounts(nextAccounts);
      ...
      setAuthError('');
      setActiveTab('home');
      notifyActivity('Admin sign in success', `${profile.username} is now connected.`);
      return;
    }
  ...
  const handlePlayGame = (game: Game) => {
    ...
    // Mark current user as playing this game
    if (userProfile) {
      const updatedAccounts = accounts.map((account) => {
        if (account.username.toLowerCase() === userProfile.username.toLowerCase()) {
          return { ...account, currentGameId: game.id };
        }
        return account;
      });
      setAccounts(updatedAccounts);
  ...
  const handleDeleteGame = (gameId: string) => {
    const nextGames = games.filter((game: Game) => game.id !== gameId);
    ...
    <div className="min-h-screen flex flex-col justify-between text-white bg-[#040307] font-sans relative overflow-x-hidden selection:bg-purple-600/50 selection:text-white">
      ...
      <main className="flex-1 relative z-10 w-full">
        ...
        {activeTab === 'admin' && isAdminUser(userProfile?.username) && (
          <AdminPanel 
            ...
            liveSocketConnections={liveSocketConnections}
          />
        )}
      ...
      {activeGame && (
        <GamePlayer 
          ...
          onClose={() => {
            setActiveGame(null);
            // Clear game from user's account when closing
            if (userProfile) {
              const updatedAccounts = accounts.map((account) => {
                if (account.username.toLowerCase() === userProfile.username.toLowerCase()) {
                  const { currentGameId, ...rest } = account;
                  return rest;
                }
                return account;
              });
